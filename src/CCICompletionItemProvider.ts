...
export class CCICompletionItemProvider implements CompletionItemProvider {
    ...
    openTextDocument(doc: TextDocument): void {
        if (doc.languageId === "html") {
            if (this.htmlDoc) {
                ...
                this.htmlDoc = new HTMLDoc(this.linter, doc, this.parser, this.cachePath);
            }
        }
    }
    ...
    changeTextDocument(event: TextDocumentChangeEvent): void {
        if (!this.htmlDoc) {
            if (event.document.languageId === "html") {
                this.htmlDoc = new HTMLDoc(this.linter, event.document, this.parser, this.cachePath);
            }
        } else {
            if (event.document.languageId === "html") {
                if (this.htmlDoc.isSameDoc(event.document)) {
                    for (const e of event.contentChanges) {
                        this.htmlDoc.editTree(changeEvent2Edit(e, this.htmlDoc));
                    }
                } else {
    ...
    provideCompletionItems(document: TextDocument, position: Position, _token: CancellationToken, _context: CompletionContext): ProviderResult<CompletionItem[]> {
        if (!this.htmlDoc) {
            this.htmlDoc = new HTMLDoc(this.linter, document, this.parser, this.cachePath);
        }
...
function changeEvent2Edit(e: TextDocumentContentChangeEvent, htmlDoc: HTMLDoc): Parser.Edit {
    return {
