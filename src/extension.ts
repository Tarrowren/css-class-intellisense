...
export function activate(context: ExtensionContext) {
    const config = workspace
        .getConfiguration()
        .get("cssClassIntellisense.remoteCSSCachePath") as string;
    ...
    context.subscriptions.push(
        linter.onDidChangeDiagnostics(e => {
            if (e.diagnostic) {
    ...
    context.subscriptions.push(
        workspace.onDidChangeTextDocument(event =>
            provider.changeTextDocument(event)
        )
    );
    context.subscriptions.push(
        workspace.onDidOpenTextDocument(doc => provider.openTextDocument(doc))
    );
    context.subscriptions.push(
